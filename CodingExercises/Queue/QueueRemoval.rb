...
Position = Struct.new(:index, :value)
def find_positions(arr, x)
    arr_ = []
    position_arr = arr.map.with_index(1){|val, index| Position.new(index, val)}
    (1..x).each{
        return arr_ if position_arr.empty?
        max_position = position_arr[0]
        arr_popped = []
        (1..x).each{|index| 
            position_arr_it = position_arr.shift
            break if position_arr_it.nil?
            if (max_position[:value] < position_arr_it[:value])
                max_position = position_arr_it
            end
            position_arr_it[:value] -= 1 if position_arr_it[:value] > 0
            arr_popped << position_arr_it
        }
        arr_ << max_position[:index]
        arr_popped.delete(max_position)
        arr_popped.each{|arr_popped_it|
            position_arr.push arr_popped_it
        }
    }
    return arr_
end
